...
for query in queries:
    ...
    if count in word_counts:
        ...
        word_counts[count] = 1
total_queries = sum(word_counts.values())
for count in sorted(word_counts.keys()):
    percent = (word_counts[count] / total_queries) * 100
    formatted_percent = "{:.2f}%".format(percent)
    print(f"Поисковых запросов, содержащих {count} слов(а): {formatted_percent}")
"""
Результат:
...
max_channel = max(stats, key=lambda x: stats[x])
...
if len(my_list) > 1:
    last_key = my_list[-2]
    current[last_key] = my_list[-1]
else:
    result[my_list[0]] = None
print(result)
"""
